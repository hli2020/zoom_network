function anchors = proposal_generate_anchors_multi(varargin)
% --------------------------------------------------------
% Zoom Network
...
ip.addParameter('base_size',        16*sqrt(2),      @isscalar);
% ratio list of anchors
ip.addParameter('ratios',           [0.5, 1, 2],     @ismatrix);
% scale list of anchors
ip.addParameter('scales',           2.^(0:5),        @ismatrix);
ip.parse(varargin{:});
opts = ip.Results;
scales = opts.scales;
...
% 3 is the level number
anchors = cell(3, 1);
interval = length(scales)/3;
for i = 1:3
    window = (1 + (i-1)*interval) : (interval + (i-1)*interval); 
    out = cellfun(@(x) scale_jitter(x, scales(window)), ...
        num2cell(ratio_anchors, 2), 'UniformOutput', false);
    anchors{i} = cat(1, out{:});
end
